...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // รับแจ้งว่ามีคนกำลังทอดเต๋า
    socket.on('player_rolling', ({ name }) => {
      setIsRolling(true);
      setDiceResult(null); // เคลียร์ผลเก่าทันทีก่อนหมุนใหม่
    });
    ...
    socket.on('dice_rolled', ({ player, result, d1, d2, isReroll }) => {
      ...
      setIsRolling(false);
      setRollingDiceFace(d1); // ให้เต๋าโชว์เลขจริงที่ได้
